...
export default function MicButton({ isRecording, isProcessing, isPlaying, disabled, onPress, onRelease }: {
  isRecording: boolean; isProcessing: boolean; isPlaying: boolean;
  disabled: boolean; onPress: () => void; onRelease: () => void;
}) {
  if (isProcessing) {
    return (
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none"
            stroke={isRecording ? "#fff" : "#fb7185"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <span className="text-xs text-patch-soft/50">{isRecording ? "Release to send" : "Hold to speak"}</span>
    </div>
  );
}
